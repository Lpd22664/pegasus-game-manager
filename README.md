...
```text
/home/acm/Projects/pegasus-game-manager/pegasus-game-manager
```
...
4. Press `Find artwork` to identify the title and fill its artwork/metadata, or choose local images.
...
The editor previews local artwork, exposes the game summary, preserves additional Pegasus asset fields, tracks unsaved changes, and prevents manual entries from being written into the sync-owned Xbox Cloud collection. `Ctrl+R` reloads the manager catalogue.
...
The tests cover Pegasus parsing/rendering, desktop-file parsing and filtering, catalogue de-duplication, platform IDs, title-match confidence, artwork enrichment/failure behavior, validation, and launcher generation.
